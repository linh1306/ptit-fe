    ...
    apiClient
      ...
      .then((response) => {
        if (response.data.status) {
          ...
          const errorMessage = response.data.error?.message || "Lỗi không xác định từ API";
...
export function createFetcher<TBody, TResponse>(url: string, method: "get" | "post" | "put" | "patch" | "delete"): FucApi<TBody, TResponse> {
...
export default async function fetcher<T>(
  ...
  return new Promise<IRes<T>>((resolve, reject) => {
    const config: AxiosRequestConfig = {
      ...
      data: options.body,
      params: { ...options.params, ...options.pagination },
    };
