...
interface ICreateColumnsOption<T> {
...
export function createColumns<T>(
  ...
  { pagination, onEdit, onDelete }: ICreateColumnsOption<T>
) {
  ...
  if (onEdit || onDelete) {
    newColumns.push({
      ...
      render: (_, record) => (
        <Space size="middle">
          {onEdit && (
            <EditOutlined className="text-blue-500" onClick={() => onEdit(record)} />
          )}
          {onDelete && (
            <DeleteOutlined className="text-red-500" onClick={() => onDelete(record)} />
          )}
        </Space>
      ),
    });
